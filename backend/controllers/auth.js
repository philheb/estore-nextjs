...
exports.signin = (req, res) => {
  ...
  User.findOne({ email }).exec((err, user) => {
    ...
    return res.json({
      token,
      user: { _id, name, email, role, imageUrl }
    });
  });
};
      ...
      if (email_verified) {
        User.findOne({ email }).exec((err, user) => {
          if (user) {
            ...
            res.cookie("token", token, { expiresIn: "1d" });
            const { _id, email, name, role, imageUrl } = user;
            return res.json({
              token,
              user: { _id, email, name, role, imageUrl }
            });
          } else {
            let password = jti;
            ...
            user.save((err, data) => {
              ...
              res.cookie("token", token, { expiresIn: "1d" });
              const { _id, email, name, role, imageUrl } = data;
              return res.json({
                token,
                user: {
                  _id,
...
exports.canUpdateAndDeleteProduct = (req, res, next) => {
  ...
  Product.findOne({ slug }).exec((err, data) => {
    ...
    let authorizedUser =
      data.createdBy._id.toString() === req.profile._id.toString();
    if (!authorizedUser) {
      return res.status(400).json({
